...
class CppLibHoldemLuts(CppWrapper):
    ...
    def __init__(self, n_boards_lut, n_cards_out_lut):
        try:
            ...
            print("C LUT library not available, using pure Python fallback.")
            from PokerRL.game._.cpp_wrappers.PythonLUT import PythonLibHoldemLuts
            self._python_lut = PythonLibHoldemLuts(n_boards_lut, n_cards_out_lut)
            self._n_boards_lut = n_boards_lut
            self._n_cards_out_lut = n_cards_out_lut
    ...
    def get_idx_2_hole_card_lut(self):
        if self._use_python_fallback:
            return self._python_lut.get_idx_2_hole_card_lut()
    ...
    def get_hole_card_2_idx_lut(self):
        if self._use_python_fallback:
            return self._python_lut.get_hole_card_2_idx_lut()
    ...
    def get_idx_2_flop_lut(self):
        if self._use_python_fallback:
            return self._python_lut.get_idx_2_flop_lut()
        lut = np.full(shape=(
    ...
    def get_idx_2_turn_lut(self):
        if self._use_python_fallback:
            return self._python_lut.get_idx_2_turn_lut()
        lut = np.full(shape=(
    ...
    def get_idx_2_river_lut(self):
        if self._use_python_fallback:
            return self._python_lut.get_idx_2_river_lut()
        lut = np.full(shape=(
    ...
    def get_1d_card(self, card_2d):
        ...
        """
        if self._use_python_fallback:
            return self._python_lut.get_1d_card(card_2d)
    ...
    def get_2d_card(self, card_1d):
        ...
        """
        if self._use_python_fallback:
            return self._python_lut.get_2d_card(card_1d)
